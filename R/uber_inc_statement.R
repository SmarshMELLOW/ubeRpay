...
#' @examples
#' uber_inc_statement( mydeliveries.df, tenQ = TRUE)

uber_inc_statement <- function( income_data, tenQ = FALSE ) {
...
.inc_call <- function( income.df ) {
  ...
  if ( nrow(income.df) == 0) {
    ...
    inc.sec <- bind_rows(
      inc.sec,
      data_frame( item = "Total", amt = sum(income.df$Total) ) )
  }
